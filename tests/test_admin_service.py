...
def test_get_admin_stats_reports_totals(api):
    ...
    blue = api.upload_image(user["access_token"], "stats-blue.png", (0, 0, 255))
    red = api.upload_image(user["access_token"], "stats-red.png", (255, 0, 0))
    ...
    async def _exercise(session):
        ...
        assert stats.trashed_images >= 1
        assert stats.total_storage_bytes > 0

    api.run_db(_exercise)


def test_retag_all_images_only_queues_active_images(api):
    user = api.register_and_login("admin-service-retag")
    active = api.upload_image(user["access_token"], "active-blue.png", (0, 0, 255))
    trashed = api.upload_image(user["access_token"], "trashed-green.png", (0, 255, 0))
    api.wait_for_image_status(str(active["id"]))
    api.wait_for_image_status(str(trashed["id"]))
    ...
    async def _exercise(session):
        queued = await admin_service.retag_all_images(session, user_id)
        assert queued == 1

        active_image = await session.get(Image, active_id)
        trashed_image = await session.get(Image, trashed_id)
        assert active_image.tagging_status == "pending"
        assert trashed_image.tagging_status == "done"

    api.run_db(_exercise)
    assert queue.get_nowait() == active_id


def test_delete_user_with_delete_images_removes_owned_images(api):
    user = api.register_and_login("admin-service-delete")
    uploaded = api.upload_image(user["access_token"], "delete-owned-image.png", (0, 0, 255))
    api.wait_for_image_status(str(uploaded["id"]))
    user_id = uuid.UUID(user["user"]["id"])
    uploaded_id = uuid.UUID(str(uploaded["id"]))

    async def _exercise(session):
        await admin_service.delete_user(session, user_id, delete_images=True)
        assert await session.get(User, user_id) is None
        assert await session.get(Image, uploaded_id) is None
...
def test_admin_service_lists_users_and_updates_user_settings(api):
    ...
    uploaded = api.upload_image(user["access_token"], "detail-blue.png", (0, 0, 255))
    api.wait_for_image_status(str(uploaded["id"]))
