...
def test_bulk_favorite_delete_and_restore_endpoints(api):
    ...
    favorite = api.client.post(
        "/images/bulk/favorite",
        headers=owner_headers,
        json={"image_ids": [str(owner_first["id"]), str(owner_second["id"]), missing_id]},
    )
    assert favorite.status_code == 200
    assert favorite.json() == {"processed": 2, "skipped": 1}

    unfavorite = api.client.request(
        "DELETE",
        "/images/bulk/favorite",
        headers=owner_headers,
        json={"image_ids": [str(owner_first["id"]), missing_id]},
    )
    ...
    bulk_delete = api.client.post(
        "/images/bulk/delete",
        headers=owner_headers,
        json={"image_ids": [str(owner_first["id"]), str(other_image["id"]), missing_id]},
    )
    assert bulk_delete.status_code == 200
    assert bulk_delete.json() == {"processed": 1, "skipped": 2}

    trash = api.client.get("/images/trash", headers=owner_headers)
...
def test_bulk_album_and_purge_endpoints(api):
    ...
    add_to_album = api.client.post(
        "/images/bulk/album",
        headers=collaborator_headers,
        json={"album_id": album_id, "image_ids": [str(first["id"]), str(second["id"]), str(uuid.uuid4())]},
    )
    ...
    remove_from_album = api.client.request(
        ...
        "/images/bulk/album",
        headers=collaborator_headers,
        json={"album_id": album_id, "image_ids": [str(second["id"]), str(uuid.uuid4())]},
    )
