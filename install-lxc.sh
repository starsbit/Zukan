...
run "docker compose -f '${INSTALL_DIR}/docker-compose.yml' --env-file '${INSTALL_DIR}/.env' pull" \
    || die "Docker image pull failed. Check whether the GHCR images are public and accessible from the container."
...
echo "  or run: pct exec ${CTID} -- docker compose -f ${INSTALL_DIR}/docker-compose.yml pull && systemctl restart zukan"
echo "══════════════════════════════════════════════════════"
echo ""
