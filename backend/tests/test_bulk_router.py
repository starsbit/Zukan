...
def test_bulk_favorite_delete_and_restore_endpoints(api):
    ...
    owner_first = api.upload_media(owner["access_token"], "bulk-owner-blue.png", (0, 0, 255))
    owner_second = api.upload_media(owner["access_token"], "bulk-owner-green.png", (0, 255, 0))
    other_image = api.upload_media(other["access_token"], "bulk-other-red.png", (255, 0, 0))
    api.wait_for_media_status(str(owner_first["id"]))
    api.wait_for_media_status(str(owner_second["id"]))
    api.wait_for_media_status(str(other_image["id"]))
    ...
    favorite = api.client.patch(
        ...
        json={"media_ids": [str(owner_first["id"]), str(owner_second["id"]), missing_id], "favorited": True},
    )
    ...
    unfavorite = api.client.patch(
        "/media",
        headers=owner_headers,
        json={"media_ids": [str(owner_first["id"]), missing_id], "favorited": False},
    )
    ...
    bulk_delete = api.client.patch(
        "/media",
        headers=owner_headers,
        json={"media_ids": [str(owner_first["id"]), str(other_image["id"]), missing_id], "deleted": True},
    )
    assert bulk_delete.status_code == 200
    assert bulk_delete.json() == {"processed": 1, "skipped": 2}

    trash = api.client.get("/media", headers=owner_headers, params={"state": "trashed"})
    ...
    bulk_restore = api.client.patch(
        "/media",
        headers=owner_headers,
        json={"media_ids": [str(owner_first["id"]), str(other_image["id"]), missing_id], "deleted": False},
    )
...
def test_bulk_album_and_delete_endpoints(api):
    ...
    first = api.upload_media(owner["access_token"], "bulk-album-blue.png", (0, 0, 255))
    second = api.upload_media(owner["access_token"], "bulk-album-green.png", (0, 255, 0))
    outsider_image = api.upload_media(outsider["access_token"], "bulk-outsider-red.png", (255, 0, 0))
    api.wait_for_media_status(str(first["id"]))
    api.wait_for_media_status(str(second["id"]))
    api.wait_for_media_status(str(outsider_image["id"]))
    ...
    add_to_album = api.client.put(
        ...
        json={"media_ids": [str(first["id"]), str(second["id"]), str(uuid.uuid4())]},
    )
    assert add_to_album.status_code == 200
    assert add_to_album.json() == {"processed": 2, "skipped": 1}

    album_media = api.client.get(f"/albums/{album_id}/media", headers=collaborator_headers)
    assert album_media.status_code == 200
    assert [item["id"] for item in album_media.json()["items"]] == [str(first["id"]), str(second["id"])]

    remove_from_album = api.client.request(
        "DELETE",
        f"/albums/{album_id}/media",
        headers=collaborator_headers,
        json={"media_ids": [str(second["id"]), str(uuid.uuid4())]},
    )
    ...
    delete = api.client.request(
        ...
        "/media",
        headers=owner_headers,
        json={"media_ids": [str(first["id"]), str(outsider_image["id"]), str(uuid.uuid4())]},
    )
    ...
    admin_delete = api.client.request(
        ...
        json={"media_ids": [str(outsider_image["id"]), str(uuid.uuid4())]},
    )
    assert admin_delete.status_code == 200
    assert admin_delete.json() == {"processed": 1, "skipped": 1}

    outsider_trash = api.client.get("/media", headers=admin_headers, params={"state": "trashed"})
...
def test_batch_patch_validation_and_empty_trash_endpoint(api):
    ...
    first = api.upload_media(owner["access_token"], "validation-blue.png", (0, 0, 255))
    second = api.upload_media(owner["access_token"], "validation-green.png", (0, 255, 0))
    api.wait_for_media_status(str(first["id"]))
    api.wait_for_media_status(str(second["id"]))
    ...
    trash_many = api.client.patch(
        "/media",
        headers=headers,
        json={"media_ids": [str(first["id"]), str(second["id"])], "deleted": True},
    )
